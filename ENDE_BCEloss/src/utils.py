...
def init_config(checkpoints_path, DEBUG, EPOCH=30, INTERVAL=1000, SAMPLE_SIZE=4, EVAL_INTERVAL_EPOCH=1):
    ...
    TRAIN_DATA_NUM = len(np.genfromtxt(
        config['TRAIN_FLIST'], dtype=np.str, encoding='utf-8'))
    print('train data number is:{}'.format(TRAIN_DATA_NUM))
    save_filename = config['MODEL_NAME']+'.pth'
    PATH_WEIDHT = os.path.join(checkpoints_path, save_filename)

    if torch.cuda.is_available():
        if os.path.exists(PATH_WEIDHT):
            data_last = torch.load(PATH_WEIDHT)
            print(PATH_WEIDHT)
            if config['APPEND'] == 1:
                last_iteration = data_last['iteration']
            else:
                last_iteration = 0
        else:
            last_iteration = 0
        print('last_iteration is {}'.format(last_iteration))
        BATCH_SIZE = config['BATCH_SIZE']
        MAX_ITERS = EPOCH * TRAIN_DATA_NUM // BATCH_SIZE + last_iteration
        if DEBUG:
            INTERVAL = 10
            config['MAX_ITERS'] = 80
            ...
            config['EVAL_INTERVAL'] = EVAL_INTERVAL_EPOCH * \
...
def stitch_images(*outputs, img_per_row=2):
    ...
    inputs = inputs_all[0]
    gap = 5
    images = [*inputs_all[0:3], *inputs_all[3], *inputs_all[4:]]
    columns = len(images)
