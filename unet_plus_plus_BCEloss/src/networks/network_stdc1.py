...
class BiSeNetSTDC1(BaseNetwork):

    def __init__(self, config, in_channels=7, init_weights=True):
        super(BiSeNetSTDC1, self).__init__()
        self.edge_detect = BiSeNetSOURCE("STDCNet813",1,pretrain_model="")
...
class BiSeNetSOURCE(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_boundary_2 and self.use_boundary_4 and self.use_boundary_8:
            results = [feat_out16, feat_out32, feat_out_sp2, feat_out_sp4, feat_out_sp8, feat_out]

        
        if (not self.use_boundary_2) and self.use_boundary_4 and self.use_boundary_8:
            results = [feat_out16, feat_out32, feat_out_sp4, feat_out_sp8, feat_out]

        if (not self.use_boundary_2) and (not self.use_boundary_4) and self.use_boundary_8:
            results = [feat_out16, feat_out32, feat_out_sp8, feat_out]
