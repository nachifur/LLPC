...
class DexiNed(nn.Module):
    ...
    def __init__(self, config, in_channels=7):
        super(DexiNed, self).__init__()
        self.edge_detect = DexiNedSOURCE(in_channels=in_channels)
...
class DexiNedSOURCE(nn.Module):
    """ Definition of the DXtrem network. """

    def __init__(self, in_channels=3):
        super(DexiNedSOURCE, self).__init__()
        self.block_1 = DoubleConvBlock(in_channels, 32, 64, stride=2,)
        ...
        self.block_cat = SingleConvBlock(
            6, 1, stride=1, use_bs=False)  # hed fusion method
