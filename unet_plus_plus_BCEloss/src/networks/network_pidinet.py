...
class PiDiNet(BaseNetwork):

    def __init__(self, config, in_channels=7, init_weights=True):
        super(PiDiNet, self).__init__()
        self.edge_detect = PiDiNetSOURCE(60, config_model(
            "carv4"), dil=24, sa=True, in_channels=in_channels)
...
class PiDiNetSOURCE(nn.Module):
    def __init__(self, inplane, pdcs, dil=True, sa=True, convert=False, in_channels=3):
        super(PiDiNetSOURCE, self).__init__()
        self.sa = sa
        if dil is not None:
        ...
        if convert:
            if pdcs[0] == 'rd':
                ...
                init_kernel_size = 3
                init_padding = 1
            self.init_block = nn.Conv2d(in_channels, self.inplane,
                                        kernel_size=init_kernel_size, padding=init_padding, bias=False)
            block_class = PDCBlock_converted
        else:
            self.init_block = Conv2d(
                pdcs[0], in_channels, self.inplane, kernel_size=3, padding=1)
